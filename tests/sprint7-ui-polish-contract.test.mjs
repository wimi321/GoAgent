...
test('TeacherRunCardPro behaves like an AI editor response instead of a fixed report card', () => {
  ...
  assert.match(app, /KataGo 读取当前局面/)
  assert.match(app, /KataGo 复核候选点/)
  assert.match(app, /teacherToolTraceSummary/)
  assert.match(app, /避免只凭一次引擎结果下结论/)
  assert.match(app, /正在读取棋盘、KataGo数据，正在开始分析。/)
  ...
  assert.match(app, /aria-label="搜索历史会话"/)
  assert.match(app, /aria-label="清除搜索"/)
  assert.match(app, /aria-label=\{`删除会话 \$\{session\.title\}`\}/)
