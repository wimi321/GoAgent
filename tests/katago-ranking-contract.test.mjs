...
test('Quick winrate graph uses KaTrain-style fast visits and refines suspected mistakes', () => {
  const katago = read('src/main/services/katago.ts')
  assert.match(katago, /QUICK_ANALYSIS_FAST_VISITS = 4/)
  assert.match(katago, /QUICK_ANALYSIS_MAX_SWEEP_VISITS = 16/)
  ...
  assert.match(agent, /const requestedVisits = numberInput\(input, 'maxVisits', 4, 1, 600\)/)
  assert.match(agent, /const sweepVisits = Math\.min\(16, Math\.max\(4, requestedVisits\)\)/)
  assert.match(agent, /const refineTopN = count > 1 \? 2 : 4/)
  assert.match(agent, /cancelKataGoAnalysis\(\{ group: 'quick' \}\)/)

  const app = read('src/renderer/src/App.tsx')
  assert.match(app, /QUICK_GRAPH_FAST_VISITS = 4/)
