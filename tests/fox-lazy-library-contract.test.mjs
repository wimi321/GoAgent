...
test('library panel communicates remote list state and keeps pagination compact', () => {
  ...
  assert.match(app, /aria-label=\{t\('gamePagination'\)\}/)
  assert.match(app, /library-rail-heading/)
  assert.match(app, /t\('importSgf'\)/)
  ...
  assert.match(app, /t\('remoteOnly'\)/)
  assert.match(app, /t\('cached'\)/)
  assert.match(app, /game-row__delete/)
  assert.match(app, /t\('deleteGame'\)/)
