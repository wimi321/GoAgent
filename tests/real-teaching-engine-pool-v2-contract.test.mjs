...
test('real teaching eval uses real KataGo and real LLM configuration', () => {
  ...
  assert.match(script, /GOMENTOR_REAL_EVAL/)
  assert.match(script, /GOMENTOR_KATAGO_BIN/)
  assert.match(script, /GOMENTOR_KATAGO_CONFIG/)
  assert.match(script, /GOMENTOR_KATAGO_MODEL/)
  assert.match(script, /GOMENTOR_LLM_BASE_URL/)
  assert.match(script, /GOMENTOR_LLM_API_KEY|OPENAI_API_KEY/)
...
test('persistent KataGo engine pool can be enabled without removing spawn fallback', () => {
  ...
  assert.match(persistent, /GOMENTOR_KATAGO_ENGINE_POOL/)
  ...
  assert.match(katago, /const child = spawn\(command\[0\], command\.slice\(1\)/)
})
