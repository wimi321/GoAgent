...
export function renderBoardText(record: BoardTextRecord, uptoMoveNumber: number): string {
  const boardSize = record.boardSize
  const safeMoveNumber = Math.max(0, Math.min(Math.trunc(uptoMoveNumber), record.moves.length))
  const boardState = buildBoardState({ boardSize, moves: record.moves, uptoMoveNumber: safeMoveNumber, initialStones: record.initialStones })
  const snapshot = boardStateToSnapshot(boardState)
  const stoneMap = new Map<string, StoneColor>()
  for (const stone of snapshot) {
    const coord = gtpToCoord(stone.point, boardSize)
    if (coord) stoneMap.set(`${coord.row},${coord.col}`, stone.color)
  }
  const lastMove = safeMoveNumber > 0 ? record.moves[safeMoveNumber - 1] : undefined
  const lastMoveKey = lastMove && !lastMove.pass && lastMove.row !== null && lastMove.col !== null ? `${lastMove.row},${lastMove.col}` : ''
  const columns = Array.from({ length: boardSize }, (_, col) => coordToGtp(0, col, boardSize).replace(/\d+$/, ''))
  const lines: string[] = []
  ...
  for (let row = 0; row < boardSize; row += 1) {
    ...
    for (let col = 0; col < boardSize; col += 1) {
      ...
      if (isLastMove) cells.push(color === 'B' ? '◉' : '◎')
      else if (color === 'B') cells.push('●')
      else if (color === 'W') cells.push('○')
      else cells.push('·')
    }
