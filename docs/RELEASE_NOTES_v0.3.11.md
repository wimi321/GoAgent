...
| 平台 / 场景 | 推荐下载 |
| --- | --- |
| macOS Apple Silicon | `GoMentor-0.3.11-mac-arm64.dmg` |
| macOS Intel | `GoMentor-0.3.11-mac-x64.dmg` |
| Windows 普通版，OpenCL 推荐包 | `GoMentor-0.3.11-win-x64.exe` 或 `GoMentor-0.3.11-win-x64-portable.zip` |
| Windows NVIDIA 专版，适合 NVIDIA 显卡和 CUDA 环境 | `GoMentor-0.3.11-win-x64-nvidia.exe` 或 `GoMentor-0.3.11-win-x64-nvidia-portable.zip` |
| 校验文件 | `SHA256SUMS.txt` |

### 本版重点

- 棋谱库新增删除棋谱：删除前确认，删除 GoMentor 管理目录中的缓存 SGF，并同步清理学生绑定和本地胜率分析缓存。
...
| macOS Apple Silicon | `GoMentor-0.3.11-mac-arm64.dmg` |
| macOS Intel | `GoMentor-0.3.11-mac-x64.dmg` |
| Windows 一般版，OpenCL 推薦包 | `GoMentor-0.3.11-win-x64.exe` 或 `GoMentor-0.3.11-win-x64-portable.zip` |
| Windows NVIDIA 專版 | `GoMentor-0.3.11-win-x64-nvidia.exe` 或 `GoMentor-0.3.11-win-x64-nvidia-portable.zip` |
| 校驗檔 | `SHA256SUMS.txt` |

### 本版重點

- 棋譜庫新增刪除棋譜：刪除前確認，清理 GoMentor 管理目錄中的 SGF、學生綁定與本地勝率分析快取。
...
| macOS Apple Silicon | `GoMentor-0.3.11-mac-arm64.dmg` |
| macOS Intel | `GoMentor-0.3.11-mac-x64.dmg` |
| Standard Windows x64, OpenCL recommended | `GoMentor-0.3.11-win-x64.exe` or `GoMentor-0.3.11-win-x64-portable.zip` |
| Windows NVIDIA edition for NVIDIA GPUs and CUDA runtimes | `GoMentor-0.3.11-win-x64-nvidia.exe` or `GoMentor-0.3.11-win-x64-nvidia-portable.zip` |
| Checksums | `SHA256SUMS.txt` |

### Why update

- The library can now delete games safely: confirmation first, removal of GoMentor-managed SGF cache files, student binding cleanup, and local evaluation cache cleanup.
...
| macOS Apple Silicon | `GoMentor-0.3.11-mac-arm64.dmg` |
| macOS Intel | `GoMentor-0.3.11-mac-x64.dmg` |
| Windows 標準版、OpenCL 推奨 | `GoMentor-0.3.11-win-x64.exe` または `GoMentor-0.3.11-win-x64-portable.zip` |
| NVIDIA GPU / CUDA 向け Windows NVIDIA 版 | `GoMentor-0.3.11-win-x64-nvidia.exe` または `GoMentor-0.3.11-win-x64-nvidia-portable.zip` |
| チェックサム | `SHA256SUMS.txt` |

### 主な修正

- 棋譜ライブラリで安全に削除できるようになりました。確認後、GoMentor 管理下の SGF、学生紐付け、ローカル評価キャッシュを整理します。
...
| macOS Apple Silicon | `GoMentor-0.3.11-mac-arm64.dmg` |
| macOS Intel | `GoMentor-0.3.11-mac-x64.dmg` |
| Windows 표준 x64, OpenCL 권장 | `GoMentor-0.3.11-win-x64.exe` 또는 `GoMentor-0.3.11-win-x64-portable.zip` |
| NVIDIA GPU / CUDA용 Windows NVIDIA 에디션 | `GoMentor-0.3.11-win-x64-nvidia.exe` 또는 `GoMentor-0.3.11-win-x64-nvidia-portable.zip` |
| 체크섬 | `SHA256SUMS.txt` |

### 핵심 수정

- 기보 라이브러리에서 안전 삭제를 지원합니다. 확인 후 GoMentor가 관리하는 SGF, 학생 연결, 로컬 평가 캐시를 정리합니다.
...
| macOS Apple Silicon | `GoMentor-0.3.11-mac-arm64.dmg` |
| macOS Intel | `GoMentor-0.3.11-mac-x64.dmg` |
| Windows x64 รุ่นมาตรฐาน แนะนำ OpenCL | `GoMentor-0.3.11-win-x64.exe` หรือ `GoMentor-0.3.11-win-x64-portable.zip` |
| Windows NVIDIA edition สำหรับ NVIDIA GPU และ CUDA | `GoMentor-0.3.11-win-x64-nvidia.exe` หรือ `GoMentor-0.3.11-win-x64-nvidia-portable.zip` |
| Checksums | `SHA256SUMS.txt` |

### สิ่งที่แก้ไข

- คลัง SGF สามารถลบเกมได้อย่างปลอดภัย โดยยืนยันก่อน แล้วล้างไฟล์ SGF ที่ GoMentor จัดการ ข้อมูลผู้เรียนที่ผูกไว้ และแคชการประเมินในเครื่อง
...
| macOS Apple Silicon | `GoMentor-0.3.11-mac-arm64.dmg` |
| macOS Intel | `GoMentor-0.3.11-mac-x64.dmg` |
| Windows x64 tiêu chuẩn, khuyến nghị OpenCL | `GoMentor-0.3.11-win-x64.exe` hoặc `GoMentor-0.3.11-win-x64-portable.zip` |
| Windows NVIDIA edition cho GPU NVIDIA và CUDA | `GoMentor-0.3.11-win-x64-nvidia.exe` hoặc `GoMentor-0.3.11-win-x64-nvidia-portable.zip` |
| Checksums | `SHA256SUMS.txt` |

### Bản sửa lỗi

- Thư viện SGF nay có thể xóa ván an toàn: xác nhận trước, dọn tệp SGF do GoMentor quản lý, liên kết học viên và bộ nhớ đệm đánh giá cục bộ.
...
- `pnpm typecheck`
- `pnpm build`
- Isolated Electron user-flow smoke with a temporary `GOMENTOR_APP_HOME`: delete cancel keeps the row, delete confirm removes the row and managed SGF file.
- `pnpm check:release-notes-i18n`

## Known Notes

- Delete only removes SGF files under GoMentor's managed app home. External source files imported by the user are not touched directly.
