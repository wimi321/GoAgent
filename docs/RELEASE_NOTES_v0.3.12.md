...
| 平台 / 场景 | 推荐下载 |
| --- | --- |
| macOS Apple Silicon | `GoAgent-0.3.12-mac-arm64.dmg` |
| macOS Intel | `GoAgent-0.3.12-mac-x64.dmg` |
| Windows 普通版，OpenCL 推荐包 | `GoAgent-0.3.12-win-x64.exe` 或 `GoAgent-0.3.12-win-x64-portable.zip` |
| Windows NVIDIA 专版，适合 NVIDIA 显卡和 CUDA 环境 | `GoAgent-0.3.12-win-x64-nvidia.exe` 或 `GoAgent-0.3.12-win-x64-nvidia-portable.zip` |
| 校验文件 | `SHA256SUMS.txt` |
...
| macOS Apple Silicon | `GoAgent-0.3.12-mac-arm64.dmg` |
| macOS Intel | `GoAgent-0.3.12-mac-x64.dmg` |
| Windows 一般版，OpenCL 推薦包 | `GoAgent-0.3.12-win-x64.exe` 或 `GoAgent-0.3.12-win-x64-portable.zip` |
| Windows NVIDIA 專版 | `GoAgent-0.3.12-win-x64-nvidia.exe` 或 `GoAgent-0.3.12-win-x64-nvidia-portable.zip` |
| 校驗檔 | `SHA256SUMS.txt` |
...
| macOS Apple Silicon | `GoAgent-0.3.12-mac-arm64.dmg` |
| macOS Intel | `GoAgent-0.3.12-mac-x64.dmg` |
| Standard Windows x64, OpenCL recommended | `GoAgent-0.3.12-win-x64.exe` or `GoAgent-0.3.12-win-x64-portable.zip` |
| Windows NVIDIA edition for NVIDIA GPUs and CUDA runtimes | `GoAgent-0.3.12-win-x64-nvidia.exe` or `GoAgent-0.3.12-win-x64-nvidia-portable.zip` |
...
- The AI teacher full-game KataGo tool now uses GoAgent's native quick-analysis path instead of the Python report path.
...
| macOS Apple Silicon | `GoAgent-0.3.12-mac-arm64.dmg` |
| macOS Intel | `GoAgent-0.3.12-mac-x64.dmg` |
| Windows 標準版、OpenCL 推奨 | `GoAgent-0.3.12-win-x64.exe` または `GoAgent-0.3.12-win-x64-portable.zip` |
| NVIDIA GPU / CUDA 向け Windows NVIDIA 版 | `GoAgent-0.3.12-win-x64-nvidia.exe` または `GoAgent-0.3.12-win-x64-nvidia-portable.zip` |
...
- AI teacher の全局 KataGo ツールは Python レポートではなく GoAgent のネイティブ高速分析を使います。
...
| macOS Apple Silicon | `GoAgent-0.3.12-mac-arm64.dmg` |
| macOS Intel | `GoAgent-0.3.12-mac-x64.dmg` |
| Windows 표준 x64, OpenCL 권장 | `GoAgent-0.3.12-win-x64.exe` 또는 `GoAgent-0.3.12-win-x64-portable.zip` |
| NVIDIA GPU / CUDA용 Windows NVIDIA 에디션 | `GoAgent-0.3.12-win-x64-nvidia.exe` 또는 `GoAgent-0.3.12-win-x64-nvidia-portable.zip` |
| 체크섬 | `SHA256SUMS.txt` |
...
- AI teacher의 전체 기보 KataGo 도구는 Python 보고서 경로 대신 GoAgent 네이티브 빠른 분석을 사용합니다.
...
| macOS Apple Silicon | `GoAgent-0.3.12-mac-arm64.dmg` |
| macOS Intel | `GoAgent-0.3.12-mac-x64.dmg` |
| Windows x64 รุ่นมาตรฐาน แนะนำ OpenCL | `GoAgent-0.3.12-win-x64.exe` หรือ `GoAgent-0.3.12-win-x64-portable.zip` |
| Windows NVIDIA edition สำหรับ NVIDIA GPU และ CUDA | `GoAgent-0.3.12-win-x64-nvidia.exe` หรือ `GoAgent-0.3.12-win-x64-nvidia-portable.zip` |
| Checksums | `SHA256SUMS.txt` |
...
- เครื่องมือ KataGo ทั้งเกมของ AI teacher ใช้เส้นทาง quick-analysis ของ GoAgent แทนรายงาน Python
...
| macOS Apple Silicon | `GoAgent-0.3.12-mac-arm64.dmg` |
| macOS Intel | `GoAgent-0.3.12-mac-x64.dmg` |
| Windows x64 tiêu chuẩn, khuyến nghị OpenCL | `GoAgent-0.3.12-win-x64.exe` hoặc `GoAgent-0.3.12-win-x64-portable.zip` |
| Windows NVIDIA edition cho GPU NVIDIA và CUDA | `GoAgent-0.3.12-win-x64-nvidia.exe` hoặc `GoAgent-0.3.12-win-x64-nvidia-portable.zip` |
| Checksums | `SHA256SUMS.txt` |
...
- Công cụ KataGo toàn ván của AI teacher dùng quick-analysis gốc của GoAgent thay vì đường dẫn báo cáo Python.
...
- Real Windows smoke: created `C:\Users\kk\.goagent\runtime\venv`, installed `sgfmill==1.1.1`, and confirmed `sgfmill ok`.
