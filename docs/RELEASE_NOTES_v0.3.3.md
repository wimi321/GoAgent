...
GoAgent v0.3.3 improves the current-move teacher explanation quality. The main change is “teaching pacing”: routine joseki should be short, joseki branches should show key variations, and middle-game fights should explain purpose, expected reply, continuation, and practical result.

QQ群：1030632742，欢迎一起交流、提建议、完善 GoAgent。

## Downloads

- macOS Apple Silicon: `GoAgent-0.3.3-mac-arm64.dmg`
- macOS Intel: `GoAgent-0.3.3-mac-x64.dmg`
- Windows x64 portable: `GoAgent-0.3.3-win-x64-portable.zip`
- Windows x64 installer: `GoAgent-0.3.3-win-x64.exe`
...
- Windows 用户优先下载 `GoAgent-0.3.3-win-x64-portable.zip` 免安装版。
- macOS Apple Silicon 下载 `GoAgent-0.3.3-mac-arm64.dmg`。
- macOS Intel 下载 `GoAgent-0.3.3-mac-x64.dmg`。
