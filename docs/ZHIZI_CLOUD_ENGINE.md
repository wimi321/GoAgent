...
当前智子开放文档没有提供可嵌入 GoAgent 的订单、支付、充值或余额 API，也没有稳定的公开充值直链。因此 GoAgent 不做应用内支付。

如果检测连接时提示 `额度不足`、`没有可用算力` 或 `暂无 worker`：

1. 点击设置页里的 `官方开通说明`。
2. GoAgent 会打开 `http://www.zhizigo.cn/faq#pricing`，这里是智子官方 FAQ 的算力价格说明。
3. 按智子官方页面、QQ 群或官方客户端指引完成开通/购买。
4. 回到 GoAgent，点击 `刷新远程会话` 或 `检测连接`。

拿到智子官方正式充值/购买链接后，只需要替换 GoAgent 的官方开通说明 URL，不需要改远程分析协议。
...
- `智子云余额不足或当前没有可用算力`：先查看 `官方开通说明`，确认套餐或余额已经生效；如果刚充值，点击 `刷新远程会话` 或 `检测连接`。
