...
GoAgent v0.3.20 focuses on more grounded teaching evidence and a more reliable voice experience. It adds explicit score perspective summaries so the teacher does not misread KataGo scoreLead signs when explaining who is ahead by points. It also includes the KataGo Trace Translator, Volcengine / Doubao TTS provider, and the v0.3.19 evidence and voice reliability work.
...
| 平台 / 场景 | 推荐下载 |
| --- | --- |
| macOS Apple Silicon | `GoAgent-0.3.20-mac-arm64.dmg` |
| macOS Intel | `GoAgent-0.3.20-mac-x64.dmg` |
| Windows 普通版，OpenCL 推荐包 | `GoAgent-0.3.20-win-x64.exe` 或 `GoAgent-0.3.20-win-x64-portable.zip` |
| Windows NVIDIA 专版，适合 NVIDIA 显卡和 CUDA 环境 | `GoAgent-0.3.20-win-x64-nvidia.exe` 或 `GoAgent-0.3.20-win-x64-nvidia-portable.zip` |
| 校验文件 | `SHA256SUMS.txt` |

### 本版重点

- 修复 LLM 讲胜负和目数时可能把 `scoreLead` 正负号理解错的问题；老师现在会使用 `scoreSummary` 里的“黑/白领先”和领先目数。
...
| macOS Apple Silicon | `GoAgent-0.3.20-mac-arm64.dmg` |
| macOS Intel | `GoAgent-0.3.20-mac-x64.dmg` |
| Windows 一般版，OpenCL 推薦包 | `GoAgent-0.3.20-win-x64.exe` 或 `GoAgent-0.3.20-win-x64-portable.zip` |
| Windows NVIDIA 專版 | `GoAgent-0.3.20-win-x64-nvidia.exe` 或 `GoAgent-0.3.20-win-x64-nvidia-portable.zip` |
| 校驗檔 | `SHA256SUMS.txt` |

### 本版重點

- 修復 LLM 講勝負與目數時可能誤解 `scoreLead` 正負號的問題；老師現在會使用 `scoreSummary` 中明確的黑/白領先與目數。
...
| macOS Apple Silicon | `GoAgent-0.3.20-mac-arm64.dmg` |
| macOS Intel | `GoAgent-0.3.20-mac-x64.dmg` |
| Standard Windows x64, OpenCL recommended | `GoAgent-0.3.20-win-x64.exe` or `GoAgent-0.3.20-win-x64-portable.zip` |
| Windows NVIDIA edition for NVIDIA GPUs and CUDA runtimes | `GoAgent-0.3.20-win-x64-nvidia.exe` or `GoAgent-0.3.20-win-x64-nvidia-portable.zip` |
| Checksums | `SHA256SUMS.txt` |

### Why update

- Fixes score and winner wording when the LLM explains point leads: the teacher now uses explicit `scoreSummary` leader and lead-point fields instead of inferring from a bare `scoreLead` sign.
...
| macOS Apple Silicon | `GoAgent-0.3.20-mac-arm64.dmg` |
| macOS Intel | `GoAgent-0.3.20-mac-x64.dmg` |
| Windows 標準版、OpenCL 推奨 | `GoAgent-0.3.20-win-x64.exe` または `GoAgent-0.3.20-win-x64-portable.zip` |
| NVIDIA GPU / CUDA 向け Windows NVIDIA 版 | `GoAgent-0.3.20-win-x64-nvidia.exe` または `GoAgent-0.3.20-win-x64-nvidia-portable.zip` |
| チェックサム | `SHA256SUMS.txt` |

### 主な変更

- LLM が `scoreLead` の符号を取り違えて勝敗や目差を説明する問題を修正。教師は `scoreSummary` の勝っている色と目数を使います。
...
| macOS Apple Silicon | `GoAgent-0.3.20-mac-arm64.dmg` |
| macOS Intel | `GoAgent-0.3.20-mac-x64.dmg` |
| Windows 표준 x64, OpenCL 권장 | `GoAgent-0.3.20-win-x64.exe` 또는 `GoAgent-0.3.20-win-x64-portable.zip` |
| NVIDIA GPU / CUDA용 Windows NVIDIA 에디션 | `GoAgent-0.3.20-win-x64-nvidia.exe` 또는 `GoAgent-0.3.20-win-x64-nvidia-portable.zip` |
| 체크섬 | `SHA256SUMS.txt` |

### 이번 버전

- LLM이 `scoreLead` 부호를 잘못 해석해 승부나 집 차이를 설명할 수 있던 문제를 수정했습니다. 이제 교사는 `scoreSummary`의 선두 색과 집 차이를 사용합니다.
...
| macOS Apple Silicon | `GoAgent-0.3.20-mac-arm64.dmg` |
| macOS Intel | `GoAgent-0.3.20-mac-x64.dmg` |
| Windows x64 มาตรฐาน แนะนำ OpenCL | `GoAgent-0.3.20-win-x64.exe` หรือ `GoAgent-0.3.20-win-x64-portable.zip` |
| Windows NVIDIA edition สำหรับ NVIDIA GPU และ CUDA | `GoAgent-0.3.20-win-x64-nvidia.exe` หรือ `GoAgent-0.3.20-win-x64-nvidia-portable.zip` |
| Checksums | `SHA256SUMS.txt` |

### จุดสำคัญของรุ่นนี้

- แก้ปัญหา LLM ตีความเครื่องหมาย `scoreLead` ผิดเวลาพูดว่าใครนำกี่แต้ม โดยครูจะใช้ข้อมูลผู้นำและแต้มจาก `scoreSummary` โดยตรง
...
| macOS Apple Silicon | `GoAgent-0.3.20-mac-arm64.dmg` |
| macOS Intel | `GoAgent-0.3.20-mac-x64.dmg` |
| Windows x64 tiêu chuẩn, khuyến nghị OpenCL | `GoAgent-0.3.20-win-x64.exe` hoặc `GoAgent-0.3.20-win-x64-portable.zip` |
| Windows NVIDIA edition cho GPU NVIDIA và CUDA | `GoAgent-0.3.20-win-x64-nvidia.exe` hoặc `GoAgent-0.3.20-win-x64-nvidia-portable.zip` |
| Checksums | `SHA256SUMS.txt` |

### Điểm mới

- Sửa lỗi LLM có thể hiểu sai dấu của `scoreLead` khi nói bên nào đang dẫn bao nhiêu điểm; giáo viên giờ dùng trực tiếp `scoreSummary` với bên dẫn và số điểm dẫn.
